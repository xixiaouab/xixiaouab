<div align="center">

# Hi, I'm Xi Xiao

Ph.D. Student in Computer Science @ UAB  
Student Researcher @ Oak Ridge National Laboratory · Applied Scientist Intern @ AWS AI Labs

[![Homepage](https://img.shields.io/badge/Homepage-xixiaouab.github.io-0A5C36?style=for-the-badge&logo=githubpages&logoColor=white)](https://xixiaouab.github.io/)
[![Google Scholar](https://img.shields.io/badge/Google%20Scholar-Xi%20Xiao-4285F4?style=for-the-badge&logo=googlescholar&logoColor=white)](https://scholar.google.com/citations?user=Q4DnRVgAAAAJ&hl=en)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Xi%20Xiao-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/xi-xiao-4800272a5)
[![GitHub](https://img.shields.io/badge/GitHub-xixiaouab-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/xixiaouab)

</div>

## About Me

I build efficient and reliable AI systems across language models, vision-language models, and computer vision. My recent work focuses on MLLM agents, reasoning, efficient adaptation, and foundation models for real-world scientific and medical applications.

- Core contributor to **ORBIT-2**, an exascale climate foundation model pretrained on Frontier.
- **SC 2025 Best Paper Award** and **ACM Gordon Bell Prize finalist** for ORBIT-2.
- Open to research collaborations on agents, post-training, efficient adaptation, and multimodal foundation models.

## Research Highlights

| Area | Selected Work |
| --- | --- |
| Efficient Adaptation | [StructLoRA](https://github.com/xixiaouab/StructLoRA), Sensitivity-LoRA, CTR-LoRA |
| Agents & Reasoning | [Agent Harness Engineering: A Survey](https://lnkd.in/en_4asZy), MLLM reasoning / understanding |
| Vision-Language Systems | Roadbench, Prompt-based Adaptation Survey, Visual Instance-aware Prompt Tuning |
| Scientific AI | ORBIT-2, fMRI-LM, medical image understanding and segmentation |

## GitHub Snapshot

<div align="center">

<a href="https://github.com/xixiaouab">
  <img height="165" src="./assets/profile-stats.svg" alt="Xi Xiao's GitHub stats" />
</a>
<a href="https://github.com/xixiaouab">
  <img height="165" src="./assets/research-stack.svg" alt="Research and coding stack" />
</a>

</div>

## Featured Repositories

<div align="center">

<a href="https://github.com/xixiaouab/StructLoRA">
  <img width="420" src="./assets/repo-structlora.svg" alt="StructLoRA repository card" />
</a>
<a href="https://github.com/xixiaouab/Visual-Instance-aware-Prompt-Tuning">
  <img width="420" src="./assets/repo-vipt.svg" alt="Visual Instance-aware Prompt Tuning repository card" />
</a>
<a href="https://github.com/xixiaouab/PROBE">
  <img width="420" src="./assets/repo-probe.svg" alt="PROBE repository card" />
</a>
<a href="https://github.com/xixiaouab/xixiaouab.github.io">
  <img width="420" src="./assets/repo-homepage.svg" alt="Homepage repository card" />
</a>

</div>

## Recent Focus

```text
MLLM Agents and Reasoning      ####################
Efficient Model Adaptation     ##################
Vision-Language Foundation AI  ################
Scientific and Medical AI      ###############
```

## Contact

- Website: [xixiaouab.github.io](https://xixiaouab.github.io/)
- Email: `xxiao [at] uab (dot) edu`
- Scholar: [Google Scholar](https://scholar.google.com/citations?user=Q4DnRVgAAAAJ&hl=en)
